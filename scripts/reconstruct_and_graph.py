...
def separate_duplications(lengthofdupl, loc):
    ...
    for entry in duplicationfile:
        if '-' not in entry.seq:
            copy_1 = entry.seq[:int(lengthofdupl)]
            copy_2 = entry.seq[int(lengthofdupl):]
            ...
            if len(preduplication_) in range(int(lengthofdupl)-1, int(lengthofdupl)+2 ):
                preduplication_ = preduplication_
                for i in range(0, len(preduplication_), 3):
                    preduplication[entry.id].append(preduplication_[i:i+3])

    if loc == "post_1":
        return(postduplication_1)
    if loc == "post_2":
        return(postduplication_2)
    if loc == "pre":
        return(preduplication)


def recursive_mutations(treefile, copy, typ):
    """
            ...
            for b in branch:
                if b.name in copy:
                    ...
                    for codon_branch, codon_b in zip(copy[branch.name], copy[b.name]):
                        ...
                        index+=1
    if typ == "syn": return(synonymous_)
    if typ == "nonsyn": return(nonsynonymous_)

def refine_recursive(treefile, nonsynonymous, synonymous, typ):
    """
    ...
    if typ == "syn": return(synonymous)
    if typ == "nonsyn": return(nonsynonymous)


def mutations(nonsynonymous, synonymous, typ):
    """
    ...
    if typ == "syn": return(syn_)
    if typ == "nonsyn": return(nonsyn_)
...
if __name__=="__main__":
    ...
    for branch in tree_file.get_nonterminals(order='preorder'):
        if pd.isna(branch.name) == False:
            ...
            if '-'*(int(args.length)-1) not in seq_dict[branch.name]:
    ...
    copy1 = separate_duplications(args.length, "post_1")
    copy2 = separate_duplications(args.length, "post_2")
    pre = separate_duplications(args.length, "pre")

    nonsynonymous_1 = recursive_mutations(tree_file, copy1, "nonsyn")
    synonymous_1 = recursive_mutations(tree_file, copy1, "syn")

    nonsynonymous_2 = recursive_mutations(tree_file, copy2, "nonsyn")
    synonymous_2 = recursive_mutations(tree_file, copy2, "syn")

    nonsynonymous_pre = recursive_mutations(tree_file, pre, "nonsyn")
    synonymous_pre  = recursive_mutations(tree_file, pre, "syn")

    nonsynonymous_1_refined = refine_recursive(tree_file, nonsynonymous_1, synonymous_1, "nonsyn")
    synonymous_1_refined = refine_recursive(tree_file, nonsynonymous_1, synonymous_1, "syn")


    nonsynonymous_2_refined = refine_recursive(tree_file, nonsynonymous_2, synonymous_2, "nonsyn")
    synonymous_2_refined = refine_recursive(tree_file, nonsynonymous_2, synonymous_2, "syn")

    nonsynonymous_pre_refined =  refine_recursive(tree_file, nonsynonymous_pre, synonymous_pre, "nonsyn")
    synonymous_pre_refined = refine_recursive(tree_file, nonsynonymous_pre, synonymous_pre, "syn")

    nonsyn_1 = mutations(nonsynonymous_1_refined, synonymous_1_refined, "nonsyn")
    syn_1 = mutations(nonsynonymous_1_refined, synonymous_1_refined, "syn")

    nonsyn_2 = mutations(nonsynonymous_2_refined, synonymous_2_refined, "nonsyn")
    syn_2 = mutations(nonsynonymous_2_refined, synonymous_2_refined, "syn")

    nonsyn_pre = mutations(nonsynonymous_pre_refined, synonymous_pre_refined, "nonsyn")
    syn_pre = mutations(nonsynonymous_pre_refined, synonymous_pre_refined, "syn")

    scaled_syn_pre, scaled_syn_1, scaled_syn_2, scaled_nonsyn_pre, scaled_nonsyn_1, scaled_nonsyn_2 = (dict() for i in range(6))
    
    predupl_ = [syn_pre, nonsyn_pre]
    predupl_dicts = [scaled_syn_pre, scaled_nonsyn_pre]
    ...
    cumulative_syn_1 = cumulative(scaled_syn_1)
    cumulative_syn_2 = cumulative(scaled_syn_2)
    cumulative_syn_pre = cumulative(scaled_syn_pre)
    cumulative_nonsyn_1 = cumulative(scaled_nonsyn_1)
    cumulative_nonsyn_2 = cumulative(scaled_nonsyn_2)
    cumulative_nonsyn_pre = cumulative(scaled_nonsyn_pre)
    ...
    axs[0].step(cumulative_syn_pre.keys(), cumulative_syn_pre.values(), label= f'preduplication', where='post' )
    ...
    axs[1].step(cumulative_nonsyn_pre.keys(), cumulative_nonsyn_pre.values(), label='preduplication', where='post' )
    ...
    fig_1, other = plt.subplots(1,1)
    other.step(cumulative_syn_1.keys(), cumulative_syn_1.values(), label= f'1st copy postduplication', where='post')
    other.step(cumulative_syn_2.keys(), cumulative_syn_2.values(), label=f'2nd copy postduplication', where='post' )
    other.step(cumulative_syn_pre.keys(), cumulative_syn_pre.values(), label= f'preduplication', where='post' )
    other.legend(loc='lower right')
    plt.xlabel("Location within the duplication")
    plt.ylabel("Scaled Number of Mutations")
    plt.savefig(args.tsv +"synonymous_duplication_cumulative.png")


    fig_1, other = plt.subplots(1,1)
    other.step(cumulative_nonsyn_1.keys(), cumulative_nonsyn_1.values(), label= f'1st copy postduplication', where='post')
    other.step(cumulative_nonsyn_2.keys(), cumulative_nonsyn_2.values(), label=f'2nd copy postduplication', where='post' )
    other.step(cumulative_nonsyn_pre.keys(), cumulative_nonsyn_pre.values(), label= f'preduplication', where='post' )
    other.legend(loc='lower right')
    plt.xlabel("Location within the duplication")
    plt.ylabel("Scaled Number of Mutations")
    plt.savefig(args.tsv +"nonsynonymous_duplication_cumulative.png")
